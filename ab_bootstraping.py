...
# user_id - случайное число, имитирующее идентификатор пользователя, которое можно получить при помощи библиотеки uuid:
import uuid
uuid.uuid4()
# target - случайно число, имитирующее действие пользователя (например, клик или сумму потраченных средств);
# для имитации клика генерируем случайное число от в диапазоне [0,1];

# 2. На сгенерированных данных проведите оценку эксперимента:
# постройте гистограммы полученных распределений по подгруппам;
# приведите распределения к нормальному виду при помощи метода sub-bucket'ов и Bootstrap'а;
# проведите оценку различий в выборках и их статистической значимости при помощи предложенных выше функций;
# напишите вывод - какую из версий group вы бы рекомендовали катить в production :)